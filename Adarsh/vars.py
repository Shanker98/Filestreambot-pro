...
class Var(object):
    ...
    BOT_TOKEN = str(getenv('7460820812:AAHvGoxQv2wCkM2_c3pWIdImh5anQ_TaRZQ'))
    name = str(getenv('name', 'filetolinkbot'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('-1002180351100', ''))
    PORT = int(getenv('PORT', '8080'))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '159.28.136.238'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = set(int(x) for x in os.environ.get("OWNER_ID", "5985614927").split())  
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = None
    OWNER_USERNAME = str(getenv('Pornobox7_bot', ''))
    if 'DYNO' in environ:
        ON_HEROKU = True
        ...
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', 'BIND_ADRESS:PORT', '159.28.136.238')) if not ON_HEROKU or getenv('FQDN', '159.28.136.238') else APP_NAME+'.herokuapp.com'
